...
extern esp_a2d_cb_param_t btConnectionParams;
extern int btAudioSamplingFreq;
extern btMetadata_t btMetadata;
...
static void bt_av_hdl_a2d_evt(uint16_t event, void *p_param)
{
    ...
    case ESP_A2D_AUDIO_CFG_EVT: {
        ...
        if (a2d->audio_cfg.mcc.type == ESP_A2D_MCT_SBC) {
            ...
            i2s_set_clk(I2S_NUM_1, sample_rate, 16, 2);
			btAudioSamplingFreq = sample_rate;
...
static void bt_av_hdl_avrc_evt(uint16_t event, void *p_param)
{
    ...
    case ESP_AVRC_CT_METADATA_RSP_EVT: {
		...
		{
			memcpy(&btMetadata.metadata[0][0], rc->meta_rsp.attr_text, rc->meta_rsp.attr_length);
			btMetadata.metadata[0][rc->meta_rsp.attr_length] = '\0';
			btMetadata.millisUpdated = millis();
		}
		if (rc->meta_rsp.attr_id == 0x02)
		{
			memcpy(&btMetadata.metadata[1][0], rc->meta_rsp.attr_text, rc->meta_rsp.attr_length);
			btMetadata.metadata[1][rc->meta_rsp.attr_length] = '\0';
			btMetadata.millisUpdated = millis();
		}
		if (rc->meta_rsp.attr_id == 0x04)
		{
			memcpy(&btMetadata.metadata[2][0], rc->meta_rsp.attr_text, rc->meta_rsp.attr_length);
			btMetadata.metadata[2][rc->meta_rsp.attr_length] = '\0';
			btMetadata.millisUpdated = millis();
		}
		if (rc->meta_rsp.attr_id == 0x20)
		{
			memcpy(&btMetadata.metadata[3][0], rc->meta_rsp.attr_text, rc->meta_rsp.attr_length);
			btMetadata.metadata[3][rc->meta_rsp.attr_length] = '\0';
			btMetadata.millisUpdated = millis();
		}
